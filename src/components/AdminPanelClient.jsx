...
import {
  Upload, Trash2, FileText, CheckCircle, Eye,
  GraduationCap, AlertCircle, RefreshCw, LogOut,
  Database, HardDrive
} from 'lucide-react'
...
/**
 * The admin dashboard UI itself. Access is already gated server-side by
 * src/app/admin/page.jsx (profile.role must be 'admin' or 'moderator')
 * before this ever renders — every fetch below relies on the browser's
 * normal Supabase session cookie, checked again server-side per request
 * by the API routes (never trust the client-side gate alone).
 */
export default function AdminPanelClient({ adminName, adminEmail }) {
      ...
      <div style={{ maxWidth: 800, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 24 }}>
          ...
          <div style={{ minWidth: 0 }}>
            ...
            <p style={{ fontSize: 12, color: '#7d97b8', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {adminName || adminEmail}
            </p>
          </div>
          <button onClick={loadFiles} title="تحديث" style={{ marginRight: 'auto', background: '#1c2e48', border: 'none', borderRadius: 10, padding: 8, cursor: 'pointer', color: '#7d97b8', display: 'flex' }}>
            <RefreshCw size={16} />
          </button>
          <button onClick={handleSignOut} title="تسجيل الخروج" style={{ background: '#1c2e48', border: 'none', borderRadius: 10, padding: 8, cursor: 'pointer', color: P.red, display: 'flex' }}>
            <LogOut size={16} />
          </button>
        </div>
